  ...
  cacheDetailSeksi.forEach((row, i) => {
    body.innerHTML += `
      <tr>
        ...
        <td class="border px-2 py-1">${row["Nama petugas"] || "-"}</td>
        <td class="border px-2 py-1">${row["Keterangan"] || "-"}</td>
      </tr>
    `;
  });
...
(function () {
  ...
  function setupEvents() {
    ...
    document.getElementById("btnReset")?.addEventListener("click", () => {
      ...
      if (status === "done") {
        filteredData = plottingData.filter((d) => d.status_proses === "DONE");
    ...
    document.getElementById("btnBatalTolak")?.addEventListener("click", () => {
      const modal = document.getElementById("modalTolak");
    ...
    document
      .getElementById("btnKonfirmasiTolak")
      ?.addEventListener("click", () => {
        ...
        if (!confirm("Yakin ingin menolak permohonan ini?")) {
          return;
        }
        ...
        const payload = {
          action: "tolakPlotting",

          row: selectedData.id,

          petugas: user.nama_lengkap || user.nama || user.username || "-",
          ...
          plotting_peta:
            document.getElementById("cekPlotting")?.checked || false,
        ...
        fetch(APP_CONFIG.API_WEB, {
          ...
          .then((res) => {
            if (!res.success) {
              alert(res.message || "Gagal menolak permohonan");
              return;
            }
        ...
        .then(async (res) => {
          ...
          fetch(APP_CONFIG.API_WEB, {
            ...
            .then((pdfRes) => {
              ...
              document
                .querySelectorAll(".link-pdf-plotting")
  ...
  function renderTable() {
    ...
    filteredData.forEach((d, i) => {
      ...
      tbody.insertAdjacentHTML(
        ...
        `
          <tr class="${
            disabled ? "bg-yellow-50 opacity-70" : "hover:bg-blue-50"
          }">
            <td>${i + 1}</td>
            ...
            ${
              d.status_proses === "PROCESSING"
                ...
                : d.status_proses === "DITOLAK"
                ? `<span class="badge-ditolak cursor-pointer bg-red-600 text-white px-2 py-1 text-xs rounded"
                      data-id="${d.id}">
                  DITOLAK
                </span>`
  ...
  function resetFormProses() {
    ...
    document
      .querySelectorAll(".btn-done-plotting")
      .forEach((el) => el.remove());
  }


  document
    .getElementById("btnTutupInfoTolak")
    ?.addEventListener("click", () => {
      const modal = document.getElementById("modalInfoTolak");
...
function loadDaftarPloting() {
  ...
  GlobalLoading.show("Memuat data plotting...");
...
function initFilterTanggalPloting() {
  ...
  function applyFilterTanggal() {
    ...
    const hasil = GLOBAL_DATA_PLOTING.filter((row) => {
      ...
      const cocokBulan = !bulan || bln === bulan;

      const cocokTahun = !tahun || thn === tahun;
  ...
  filterBulan.onchange = applyFilterTanggal;
  filterTahun.onchange = applyFilterTanggal;
}
...
window.initDaftarPloting = initDaftarPloting;
